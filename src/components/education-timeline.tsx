...
export default function EducationTimeline({ education }: { education: Education[] }) {
    return (
                            ...
                            <div className="relative overflow-hidden bg-surface/80 backdrop-blur-xl border border-border/50 rounded-[2.5rem] p-8 md:p-12 transition-all duration-500 ease-smooth hover:border-primary/50 shadow-lg hover:shadow-2xl transform group-hover:-translate-y-3 group-hover:translate-x-2">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center gap-6 mb-8">
                                    ...
                                    <div className="flex flex-col">
                                        <h3 className="text-2xl md:text-3xl font-extrabold text-foreground tracking-tight group-hover:text-primary transition-colors duration-normal ease-smooth mb-1">{edu.institution}</h3>
                                        <div className="flex items-center gap-3">
                                            <span className="text-primary font-bold tracking-widest uppercase text-xs border border-primary/20 bg-primary/5 px-3 py-1 rounded-full">{edu.period}</span>
                                            <span className="text-muted font-bold text-lg">{edu.degree}</span>
                                        </div>
                                        {edu.location && <span className="text-muted text-sm mt-1">{edu.location}</span>}
                                    </div>
                                </div>
                                
                                {edu.description && (
                                    <p className="text-muted text-lg md:text-xl leading-relaxed font-light whitespace-pre-wrap pl-2 border-l-2 border-primary/20 group-hover:border-primary transition-colors duration-500">{edu.description}</p>
                                )}
                            </div>
                        </div>
                    </motion.div>
